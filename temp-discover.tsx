...
export default async function DiscoverContestsPage({ searchParams }: DiscoverPageProps) {
  ...
  const statusParam = STATUS_VALUES.includes(params.status as any) ? (params.status as (typeof STATUS_VALUES)[number]) : 'active';
  const sortParam = SORT_VALUES.includes(params.sort as any) ? (params.sort as (typeof SORT_VALUES)[number]) : 'ending_soon';
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground text-base">
              Choisis un brief, filme ta vidéo et dépose ta participation pour gagner le cashprize.
            </p>
          </div>
...
async function fetchContests({
  ...
  page: number;
  pageSize: number;
}): Promise<{ contests: ContestCardData[]; total: number; profileIncomplete: boolean }> {
...
function computeEligibility({
  ...
  const platformOk = !primaryPlatform || contest.networks.length === 0 || contest.networks.includes(primaryPlatform as Platform);
  if (!platformOk) reasons.push('Plateforme differente');
  return {
    ok: reasons.length === 0,
    reasons,
  };
