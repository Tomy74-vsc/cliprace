...
export default async function ContestDetailPage({ params }: { params: { id: string } }) {
  ...
  const steps: ProgressStep[] = [
    { title: 'Cr├®er ta vid├®o', description: `Publie sur ${contest.networks.join(', ') || 'ton r├®seau'}`, state: 'completed' },
    { title: 'Ajouter les hashtags', description: '#ClipRace + hashtag de marque', state: 'current' },
    { title: 'Coller le lien', description: 'Soumets ta vid├®o dans ClipRace', state: 'upcoming' },
  ];
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-8 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Comment ├ºa marche</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        ...
        <CardContent>
          {contest.prizes && contest.prizes.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">Pas de r├®compenses d├®taill├®es. Le prize pool global sÔÇÖapplique.</p>
          )}
        </CardContent>
      </Card>
...
async function SubmissionsShowcase({ contestId }: { contestId: string }) {
  ...
  if (error || !data || data.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Exemples de vid├®os</CardTitle>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">Pas encore de soumissions approuv├®es.</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <Card>
      ...
      <CardContent className="grid md:grid-cols-3 gap-3">
        {data.map((item) => (
          ...
          >
            <div className="text-sm font-medium">{item.creator_handle || 'Cr├®ateur'}</div>
            <div className="text-xs text-muted-foreground">Views : {item.views ?? 'ÔÇö'}</div>
        ...
        {data.map((row) => (
          <div key={`${row.creator_handle}-${row.rank}`} className="flex items-center gap-3 border border-border rounded-xl px-3 py-2">
            ...
            <div className="flex-1">
              <div className="font-medium">{row.creator_handle || 'Cr├®ateur'}</div>
              <div className="text-xs text-muted-foreground">Views : {row.views ?? 'ÔÇö'}</div>
            </div>
            {row.prize_cents ? (
