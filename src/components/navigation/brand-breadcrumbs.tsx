/* Breadcrumbs contextualisés pour l'espace marque */
'use client';
...
export function BrandBreadcrumbs() {
  ...
  const items = parts.map((part, index) => {
    ...
    const href = `/${['app', 'brand', ...parts.slice(0, index + 1)].join('/')}`;
    return { label, href };
  });
  ...
  return (
    <nav aria-label="Fil d'Ariane" className="flex items-center gap-1 text-sm text-muted-foreground">
      <Link href="/app/brand/dashboard" className="hover:text-foreground transition-colors">
        Tableau de bord
      </Link>
      {items.map((item) => (
