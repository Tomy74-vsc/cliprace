...
export function SubmissionForm({ contestId, allowedPlatforms, onSuccess }: SubmissionFormProps) {
  ...
  const handleViewSubmissions = () => {
    ...
    router.push("/app/creator/submissions");
  };
                ...
                render={({ field }) => (
                  <Select value={field.value} onValueChange={field.onChange}>
                    <SelectTrigger id="platform" className={errors.platform ? "border-red-500" : ""}>
                      <SelectValue placeholder="Sélectionne une plateforme" />
                    </SelectTrigger>
                    <SelectContent>
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="max-w-sm">
          <DialogHeader className="items-center text-center space-y-3">
            <div className="flex h-14 w-14 items-center justify-center rounded-full bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600">
              <CheckCircle2 className="h-8 w-8" />
            </div>
            <DialogTitle>Participation envoyée !</DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground">
              Bien joué, ta vidéo est en attente de modération. Tu peux suivre son statut et ses
              performances dans Mes soumissions.
            </DialogDescription>
          </DialogHeader>
          <div className="mt-4 flex flex-col gap-2">
            <Button className="w-full" onClick={handleViewSubmissions}>
