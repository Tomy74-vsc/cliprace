...
import { Activity, ArrowDownRight, ArrowUpRight, BarChart3, ClipboardList, Flame, Gauge, ScrollText } from 'lucide-react';
  ...
  return (
    <section className="space-y-8">
      <AdminPageHeader
        ...
        badges={<Badge variant="secondary">Maj : {formatDateTime(data.generated_at)}</Badge>}
      />
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">Variations vs J-1 et J-7</div>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <StatCard title="Soumissions en attente" value={data.today.pending_submissions.toLocaleString()} />
          <StatCard title="Cashouts en attente" value={data.today.cashouts_pending.toLocaleString()} />
          <StatCard title="Tickets support (open/pending)" value={data.today.support_open.toLocaleString()} />
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">Priorisé + CTA rapides</div>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Badge variant="secondary">Mon travail • Modération {data.todo.my_work.moderation_claimed}</Badge>
            <Badge variant="secondary">Mon travail • Support {data.todo.my_work.support_assigned_to_me}</Badge>
            <Badge variant="secondary">Mon travail • CRM {data.todo.my_work.leads_assigned_to_me}</Badge>
          </div>
        </div>

        <AdminDashboardActionsClient items={data.todo.items as any} />
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-semibold">Santé système</div>
            <div className="text-sm text-muted-foreground">Ingestion, webhooks, files, temps de traitement</div>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-semibold">Marketing & croissance</div>
            <div className="text-sm text-muted-foreground">Concours, marques à relancer, créateurs</div>
          </div>
        </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="text-sm text-muted-foreground">Actions admin + événements produit</div>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {data.journal.audit.length === 0 ? (
                ...
                data.journal.audit.map((row: any) => (
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              {data.journal.events.length === 0 ? (
                ...
                data.journal.events.map((row: any) => (
                  <div key={row.id} className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
