...
export async function PATCH(
  ...
  try {
    ...
    await enforceAdminRateLimit(req, { route: 'admin:feature-flags:update', max: 10, windowMs: 60_000 }, user.id);
    try {
...
export async function DELETE(
  ...
  try {
    ...
    const { user } = await requireAdminPermission('settings.write');
    await enforceNotReadOnly(req, user.id);
    await enforceAdminRateLimit(req, { route: 'admin:feature-flags:delete', max: 10, windowMs: 60_000 }, user.id);
    try {
      assertCsrf(req.headers.get('cookie'), req.headers.get('x-csrf'));
    } catch (csrfError) {
      throw createError('FORBIDDEN', 'Invalid CSRF token', 403, csrfError);
