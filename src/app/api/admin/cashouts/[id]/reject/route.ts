...
export async function POST(
  ...
  try {
    ...
    await enforceAdminRateLimit(req, { route: 'admin:cashouts:reject', max: 10, windowMs: 60_000 });
    try {
    ...
    await admin.from('audit_logs').insert({
      ...
      new_values: { status: 'failed', reason: parsed.data.reason, ...(breakGlass.required ? { break_glass: breakGlass } : {}) },
